...
## Backend и CPU
- Данная подпрограмма состоит из трех модулей:
    - *input_output*
    - *translator*
    - *processor*
...
- Подпрограмма *translator* на основе созданного дерева создает ассемблерный код. Алгоритм основан на рекурсии.
    - если встречается число, то добавляется команда *push число*
    - если встречается переменная, то происходит проверка на ее инициализацию, а затем добавляется команда *push [ переменная ]* (если это инициализация, то pop [ переменная ])
...
- *Frontend*
    - src
        - *input_output.cpp*  // здесь содержатся функции распечатки дерева
        - *main.cpp*
...
- *Midlend*
    - src
        - *simplifier.cpp*       // 2 функции упрощения дерева (optimisations)
    - include
        - *simplifier.h*
- *Backend* и *CPU*
    - src
        - *input_iutput.cpp*     // функции считывания дерева и его распечтаки
        - *main.cpp*
        - *translator.cpp*       // функция создания ассемблерного кода (кодогенерация)
    - include
        - *backend.h*
    - library
        - *commands.h*
